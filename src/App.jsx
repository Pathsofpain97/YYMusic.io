...
function App() {
    ...
    const switchToCominatcha = useCallback((event) => {
        if (event) {
    ...
    const switchToZielony = useCallback((event) => {
        if (event) {
            event.preventDefault();
            event.stopPropagation();
        }
    ...
    const switchToDefaultVisualizer = useCallback((event) => {
        if (event) {
            event.preventDefault();
            event.stopPropagation();
        }
            ...
            >
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', padding: '0 10px' }}>
                        ...
                        {currentVisualizer === 'cominatcha' && audioRef.current && <CominatchaVisualizer audioRef={audioRef} />}
                        {currentVisualizer === 'zielony' && audioRef.current && <ZielonyNieJestTwoimWrogiemVisualizer audioRef={audioRef} />}
                        {currentVisualizer === 'default' && audioRef.current && <AudioVisualizer audioRef={audioRef} />}
                    </div>
                </div>
